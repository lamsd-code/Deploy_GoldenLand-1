...
public class UserController {
    ...
    public ModelAndView getNews(@ModelAttribute(SystemConstant.MODEL) UserDTO model,
        ...
        /* ===================== [FIX] DEFAULTS ===================== */
        // Nếu không truyền gì thì mặc định page = 1, size = 5
        if (page == null || page <= 0) {
            // model.getPage() là int -> chỉ cần > 0
            page = (model.getPage() > 0) ? model.getPage() : 1;   // [FIX]
        }
        if (size == null || size <= 0) {
            // model.getMaxPageItems() là int -> chỉ cần > 0
            size = (model.getMaxPageItems() > 0) ? model.getMaxPageItems() : 5;  // [FIX]
        }
        /* =================== [END FIX] DEFAULTS =================== */
        ...
        int totalItems = userService.getTotalItems(model.getSearchValue());
        int totalPages = (int) Math.ceil((double) totalItems / (double) pageSize);

        model.setListResult(users);
        model.setTotalItems(totalItems);
        model.setMaxPageItems(pageSize);   // phản ánh default 5 ra view
